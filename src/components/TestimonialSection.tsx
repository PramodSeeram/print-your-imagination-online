  ...
  return <section className="py-16 bg-cyan-900">
      <div className="container-avirva">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {TESTIMONIALS.map(testimonial => <div key={testimonial.id} className="bg-slate-800 rounded-lg p-6 shadow-sm border border-slate-700">
              ...
              <div className="flex mb-3">
                {Array.from({
              length: 5
            }, (_, i) => <Star key={i} className={`h-4 w-4 ${i < testimonial.rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600'}`} />)}
              </div>
